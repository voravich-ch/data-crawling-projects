...
def get_place_review_urls():
    client = pymongo.MongoClient()
    collection = client['beerAdvocate']['placeMetadata']
    cursor = collection.find({}, projection = {"_id": 0, "response_url": 1})
    place_review_urls = [urljoin(url['response_url'], '?view=ratings') for url in cursor]
    return place_review_urls

# Parameter preparation
place_review_urls = get_place_review_urls()
# Load credentials
load_dotenv()
...
class PlaceReviewSpider(scrapy.Spider):
    name = 'placeReview'
