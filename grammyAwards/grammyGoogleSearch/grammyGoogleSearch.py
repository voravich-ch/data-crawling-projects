...
def request_data(i):
    ...
    if response.status_code == 200:
        ...
        # Collect sidebar
        name = handle_none(code="html_page.select_one('div.SPZz6b h2').get_text(strip=True)")
        role = handle_none(code="html_page.select_one('div.wwUB2c span').get_text(strip=True)")
        website = handle_none(code="html_page.select_one('span.ellip').get_text(strip=True)")
        description = handle_none(code="html_page.select_one('div.kno-rdesc span').get_text(strip=True)")
        born = handle_none(code="html_page.find('a', text='Born').parent.find_next_sibling().get_text()")
        died = handle_none(code="html_page.find('a', text='Died').parent.find_next_sibling().get_text()")
        links = handle_none(code="html_page.select('table.RJuLSb tr')")
        if links:
            output = {}
            for link in links:
            ...
            links = output.copy()
        # Search query
        query = re.search(r'q=(.+)', urls[i]).group(1).replace('+', ' ').strip()
        # Get wiki url
        wiki_url = html_page.select_one('div.yuRUbf a[href^="https://en.wikipedia.org"]')['href']
